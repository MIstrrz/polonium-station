        ...
        public void RefreshTraits()
        {
            ...
            foreach (var (categoryId, categoryTraits) in traitGroups)
            {
                ...
                foreach (var traitProto in categoryTraits)
                {
                    ...
                    // Check if this trait has a SignatureFontComponent and extract the font so that its visible on traits screen
                    VectorFont? customFont = null;
                    if (trait.Components.TryGetComponent("SignatureFont", out var signatureFontComponent))
                    {
                        // grab that font
                        if (signatureFontComponent is Content.Shared._DV.Traits.SignatureFontComponent fontComp)
                        {
                            var fontId = fontComp.Font;
                            if (fontId != null && _prototypeManager.TryIndex<FontPrototype>(fontId, out var fontPrototype))
                            {
                                // casts resource manager to resource cache to grab font resource
                                var resourceCache = (IResourceCache)_resourceManager;
                                var fontResource = resourceCache.GetResource<FontResource>(fontPrototype.Path);
                                customFont = new VectorFont(fontResource, 14);
                            }
                        }
                    }
